 ...
 * Orchestrates the interaction between LeetCode's interface and Google Sheet synchronization.
 */
export default class LeetcodeTracker {
  /**
   * Initialize the LeetCode Tracker with required services and route monitoring.
   * Sets up problem model and route change detection.
   */
  constructor() {
  ...
  showToast(message, type = "success") {
    ...
    const rootContainer = document.getElementById(
      "leetcode-tracker-toast-container",
    );
    if (
      rootContainer &&
      !rootContainer.classList.contains("lt-theme-light") &&
      !rootContainer.classList.contains("lt-theme-dark")
    ) {
      rootContainer.classList.add(
        prefersDark ? "lt-theme-dark" : "lt-theme-light",
      );
    }
  ...
  showCommentPopup() {
    return new Promise((resolve) => {
      ...
      instruction.textContent =
        "Add notes about what u did wrong";
      instruction2.style.cssText = `
          color: #525252;
          font-size: 14px;
          margin-bottom: 16px;
          line-height: 1.5;
      ...
      textarea2.placeholder =
        "Example: I used this in place of that ";
// Function to create a status toggle
function createToggle(label, options) {
  ...
  options.forEach(opt => {
    ...
    radio.type = "radio";
    radio.name = label.replace(/\s+/g, '-').toLowerCase();
    radio.value = opt;
...
// Usage
const statusToggle = createToggle("Completion Status", ["Solved without hint", "Solved with hint"]);
const revisitToggle = createToggle("Review", ["Revisit", "No revisit"]);
      ...
      popupContent.appendChild(instruction2)
      popupContent.appendChild(textarea2)
      popupContent.appendChild(statusToggle)
      popupContent.appendChild(revisitToggle)
